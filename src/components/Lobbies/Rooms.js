...
const Players = () => {
    ...
    const handleJoin = (game, roomId) => {
        navigate(`/lobby/${game}/${roomId}`);
    };

    // Below handleJoin will update the current players of certain roomId when they join
    // const handleJoin = (game, roomId) => {
    //     // Send POST request to update current players in the room
    ...
    return (
        ...
        {roomsListArray.map((room) => {
            ...
            >
                ...
                onClick={() => handleJoin(game,roomId) }>
                {isFull ? 'Full' : 'Join'}
                </FocusedButton>
            </Stack>
            );
